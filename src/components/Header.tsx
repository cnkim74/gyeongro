...
export default async function Header() {
  ...
  const headerKeys: MessageKey[] = [
    "nav.planner", "nav.themes", "nav.sherpa", "nav.medical", "nav.partners",
    "nav.stories", "nav.community", "nav.flights", "nav.hotels", "nav.activities",
    "nav.cars", "nav.login", "nav.signup", "nav.cta", "nav.profile",
    "nav.my_trips", "nav.logout", "nav.language",
  ];
  ...
  if (session?.user?.id) {
    ...
    try {
      const result = await supabase
        .schema("next_auth")
        .from("users")
        .select("name, nickname, email, image, custom_image, avatar_preset, role, business_name")
      ...
      const fallback = await supabase
        .schema("next_auth")
        .from("users")
        .select("name, email, image, custom_image, avatar_preset, role")
        .eq("id", session.user.id)
        .maybeSingle();
    ...
    user = {
      ...
      image:
        data?.custom_image ??
        getAvatarUrl(data?.avatar_preset) ??
        data?.image ??
        session.user.image ??
        null,
      role,
