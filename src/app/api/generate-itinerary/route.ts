...
export async function POST(req: NextRequest) {
  ...
  const themeHints: Record<string, string> = {
    ...
    hiking: "도시 근교 가벼운 산행 (1일 코스). 등산로 난이도(초중상), 코스 길이·소요시간, 표고차, 하산 후 식사 좋은 곳까지. 초보도 갈 수 있는 동선 우선. 등산화·물·간식 추천. 안전상 주의(낙석·뱀·일몰 시각).",
    trekking: "다일정 종주 트레킹 (2일 이상). 코스 분할, 산장·대피소 예약 정보, 일별 거리·표고차·물 보급지, 배낭 무게·필수 장비 리스트, 야간 기온, 출입 허가·예약 시스템(국립공원). 비상시 대피로·구조 연락처 명시.",
